...
- 通过定时任务，每天8点执行提醒就诊
    - 新建模块service-task，并新建定时任务类，在类上添加`@EnableScheduling`注解，并在需要定时执行的方法上添加注解`@Scheduled(cron = "0 0 8 * * ?")`，方法中通过RabbitMQ发送到消息队列
    - 在order模块中，新建监听类，用来监听定时任务向队列中发送的消息
    - 监听到消息时，执行方法， 发送就诊通知
    - 根据当前日期和订单不为已取消状态的条件，查询出所有的订单，然后将订单的参数进行拼接，并发送到mq中，短信队列监听到有消息时会执行发送短信的操作
    - 流程：定时任务类 -> 到时间触发定时任务 -> 将定时任务发送到队列中 -> orderservice的监听器监听到队列中有消息 -> 触发就诊通知方法 -> 在就诊通知方法中，查询要通知的单据，然后再将信息发送到队列中 -> 短信的监听器监听到队列中有消息，则发送短信提醒
